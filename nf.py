...
The error you're encountering is `StreamlitSetPageConfigMustBeFirstCommandError`, which means that the `st.set_page_config` function must be called as the first Streamlit command in your script. To fix this error, make sure that `st.set_page_config` is the first Streamlit command executed in your script.

Here’s how you can correct it:

1. Move the `st.set_page_config` call to the very top of your script, right after the imports.
...
Make sure `st.set_page_config` is the first Streamlit call after the import statements. This should resolve the error you are seeing.
